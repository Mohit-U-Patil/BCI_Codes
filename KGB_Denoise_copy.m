function KGB_Denoise_copy(ch2keep, nfb, fs, segmentLength, low, high, mother_wavelet, decomposition_level, denoising_method, ThresholdRule, NoiseEstimate)
...
denoised_segments = allsegments;
second_segment = allsegments(2);
...
allEEGSignals{subjectNo} = matObj;

for i=1:nbTrials %all trials
    for j=1:nbChannels %all channels
      segment = allsegments(i);
      denoised_segments(i) =  wdenoise(segment(:,:),decomposition_level, ...
...
save('../RCSP_Toolbox_GPL/allEEGSignals_denoised.mat','allEEGSignals_denoised');
